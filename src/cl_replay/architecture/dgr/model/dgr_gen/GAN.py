...
class GAN(tf.keras.Model):
    ...
    def visualize_samples(self, num_samples, classes, filename):
        ...
        eye = np.eye(self.label_dim, k=0)
        rand_l = np.random.choice(classes, size=num_samples)
        ys = eye[rand_l]
    
        sampled = self.sample(batch_size=num_samples, ys=tf.convert_to_tensor(ys))
        sampled = np.reshape(sampled, newshape=(num_samples, self.data_dim[0], self.data_dim[1], self.data_dim[2]))
