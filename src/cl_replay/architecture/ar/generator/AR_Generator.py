        ...
        for gen_it in range(0, gen_range):
            ...
            if variants == 'yes' and xs is not None:
                ...
                if vis_batch == 'yes' and gen_it == 0:
                    visualize_data(xs_batch, None, f'{self.model.vis_path}/input', f'gen_input_T{task}')
            # -------------------------- TOPDOWN: generate from topdown signal
            elif top_down == 'yes':
                log.debug(f'topdown sampling:')
                topdown_logits, one_hot = self.model.construct_topdown_for_classifier(self.num_classes, 
                                                                                      0.95, gen_classes)
            ...
            if generate_labels == True:
                if top_down == 'yes':
                    gen_ys = one_hot  # labels come from self-generated top-down signal
                else:
                    # query model with generated samples
                    query_ys = self.model(npx).numpy()
                    query_ys = np.argmax(query_ys, axis=1)
                    # log.debug(f'\nlabels gathered from model-query:\n{generated_scalar_labels})
                    # form one-hot representation of a sampled batch with dims (N,10)
                    gen_ys = np.zeros([sbs, self.num_classes], dtype=self.dtype_np_float)
                    gen_ys[range(0, sbs), query_ys] = 1
            ...
            if vis_gen == 'yes' and gen_it == 0:
                visualize_data(gen_xs, gen_ys, f'{self.model.vis_path}/gen', f'gen_output_T{task}')
