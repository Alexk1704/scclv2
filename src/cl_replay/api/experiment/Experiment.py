...
class Experiment:
    ...
    def _init_parser(self, **kwargs):
        ...
        self.vis_path           = self.parser.add_argument('--vis_path',            type=str,   default=default_path,                       help='path for visualization results')
        ...
        self.eval_during_train  = self.parser.add_argument('--eval_during_train',   type=str,   default='no', choices=['no', 'yes'],        help='log eval metrics during training for "eval_each_n_steps".')
        self.eval_each_n_steps  = self.parser.add_argument('--eval_each_n_steps',   type=int,   default=0,                                  help='perform evaluation logging each N steps (only batch-mode supported).')
    ...
    def _init_variables(self):
        ...
        self.train_set      = [None]  # stores current task training dataset
        self.train_iters    = [None]  # store current task iterations
        self.test_sets      = [None]  # stores test set
        self.test_iters     = [None]  # store test set iterations


    def _load_train_data(self, current_task):
        train, _, samples_train, _ = self.dataset.get_dataset(task_data=self.tasks[current_task], task_info=current_task)
        self.train_set = train
        self.current_task_iters = int(math.ceil(samples_train // self.batch_size))
        

    def get_task_iters(self):
        ''' returns current training task iterations. '''
        return self.current_task_iters   


    def _load_test_data(self, current_task=1, preload_past_data=False):
        ''' 
        Create the sub-task datasets and append them to the global training and testing lists.
            * Parameters:
                - current_task: 1-based int
                - preload_past_data: bool flag if also loading past data (for checkpointing)
        '''
        if self.load_task > 0 and preload_past_data:  # if a checkpoint is loaded, preload the already processed datasets for full evaluation:
            log.info(f'preloading test datasets from T1 to T{current_task}...')
            for task in range(1, current_task):
                _ = self._load_task_dataset(current_task=task, preload_past_data=False, preloading=True)
        
        _, test, _, samples_test  = self.dataset.get_dataset(task_data=self.tasks[current_task], task_info=current_task)
        test_iters      = samples_test // self.test_batch_size
        self.test_iters += [test_iters]
        self.test_sets  += [test]

        log.info(f'loaded test set for T{current_task}), contained classes/proportions: {self.tasks[current_task]}, sample size: {samples_test}')


    def get_test_iters(self, task):
        ''' returns test iterations for a given sub-task. '''
        return self.test_iters[task]
    ...
    def run_experiment(self):
        ...
        if self.full_eval == 'yes':                                             # pre-loads ALL test datasets beforehand for a complete evaluation
            for task in range(1, self.num_tasks+1):
                self._load_test_data(task, preload_past_data=False)
        ...
        for task in range(self.start_task, self.num_tasks+1):                   # loop over all tasks (T1,T2,...,TX)
            if self.full_eval == 'no':
                ...
                self._load_test_data(task, preload_past_data=pre_load)          # load dataset for current & previous tasks if a model is loaded
            self._load_train_data(task)
    ...
    def train_on_task(self, current_task):
        ''' This function runs several training steps for the current task on a TF dataset iterator or a numPy array. '''
        ...
        if self.ml_paradigm == 'unsupervised': 
            xs = self.train_set, ys = []
        else:
            xs = self.train_set[0]; ys = self.train_set[1]

        _ = self.model.fit(
            x=xs,y=ys,
            epochs=epochs,
            batch_size=self.batch_size,
            steps_per_epoch=(self.train_iters // epochs),
    ...
    def store_results(self):
        ''' Logs the results if an explicit abspath has been provided, if not, let the model handle the storage of results via, e.g., callbacks. '''
        ...
        for col,val in self.test_results.items():
            if type(val) is type(1.0) or type(val)== type(1): val = np.array([val])
            if type(val) is type(np.zeros([1])): val = val.ravel()
            if len(val) > longest: longest = len(val)
            data[col] = val
    ...
    def plot_dot(self):
        ...
        plot_model(self.model, to_file=f'{self.model.vis_path}/model_plot.png', show_shapes=True, show_layer_names=True, rankdir="TB", dpi=96)
