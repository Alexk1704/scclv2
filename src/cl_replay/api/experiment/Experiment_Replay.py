...
class Experiment_Replay(Experiment):
    ...
    def feed_sampler(self, task, current_data):
        ...
        # INFO: comment in if you want to visualize a random subset of the current dataset.
        # """ 
        if self.vis_batch == 'yes':
            rnd_indices = np.random.random_integers(0, cur_xs.shape[0], self.batch_size)
            img_data = cur_xs[rnd_indices]
            label_data = None
            if self.ml_paradigm == 'supervised':
                label_data = cur_ys[rnd_indices]
            visualize_data(img_data, label_data, f'{self.vis_path}/input', f'xs_input_T{task}')
        # """
        ...
        if task > 1:
            ...
            self.train_steps = math.ceil(self.get_task_iters() * factor)

            log.info(f'setting up "steps_per_epoch"... iterations for current task: {self.get_task_iters()}, '
    ...
    def before_task(self, task, **kwargs):
        ...
        if task > 1:
            self.generated_dataset = self.generate(
                task, data=self.train_set, gen_classes=self.past_classes, real_classes=self.real_classes
            )
            self.sampler.reset()
    ...
    def train_on_task(self, current_task):
        ...
        if self.train_method == 'batch':
            ...
            # def convert_py_generator():
            #     dataset = tf.data.Dataset.from_generator(
            #         lambda: self.sampler(),
            #         output_signature=(
            #             tf.TensorSpec(shape=(self.sampler.batch_size, *self.sampler.sample_size), dtype=tf.float32),  # xs
            #             tf.TensorSpec(shape=(self.sampler.batch_size, self.sampler.label_size),  dtype=tf.float32),   # ys
            #             tf.TensorSpec(shape=(self.sampler.batch_size), dtype=tf.float32)                              # sw
            #         )
            #     )
            #     return dataset.shuffle(buffer_size=100).prefetch(tf.data.AUTOTUNE)
            # data_ = convert_py_generator()
    ...
    def train_batch_mode(self, data, epochs=100, train_steps=1000, current_task=None, use_callbacks=True):
        ...
        for t_e in range(0, int(epochs[(current_task-1)])):
            ...
            for t_s in range(0, int(train_steps)):                          
                ...
                xs_data, targets, sw = batch_data[0], batch_data[1], batch_data[2]
                ...
                #-------------------------- METRICS EVAL
                if self.eval_each_n_steps > 0:
                    if t_s % self.eval_each_n_steps == 0:
                        ...
                        if self.eval_during_train == 'yes':
                            self._test(current_task, current_step=global_iter)
    ...
    def test_step(self, xs, ys, test_steps, test_classes=[], test_task='none'):
        # log.debug(f'xs: {xs.shape} ys: {ys.shape} cls: {ys.sum(axis=0)}\n')
        ...
        log.info(f'[TEST] -> {test_task} {test_classes}')
    ...
    def test_batch_mode(self, current_task, current_step, full_eval='no'):
        #-------------------------- SINGLE CLASS TEST
        if self.ml_paradigm == 'supervised' and self.single_class_test == 'yes':
            log.info('{:19s}'.format(' [SINGLE CLASS TEST] ').center(64, '~'))
            for test_task in range(0, self.single_cls_test.__len__()):
                ...
                self.test_step(xs, ys, test_steps, [test_task], task_name)
        ...
        for test_task in range(0 if self.DAll != [] else 1, task_end):  # test: DAll, all previous, the current & (opt.) all future sub-tasks -> 0: DAll, 1: T1, 2: T2, ...
            if hasattr(self, 'adaptor'):
                if self.op_mode == 'train' and test_task == 0:
                    continue
            ...
            test_steps      = (self.samples_test_D_ALL // self.test_batch_size) if test_task == 0 else self.get_test_iters(test_task)
        
            xs, ys          = test_dataset
            self.test_step(xs, ys, test_steps, task_classes, task_name)
